  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <Loading size="lg" text="Loading service…" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="mx-auto max-w-4xl px-6 py-10">
        <Link to="/services" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-indigo-400 transition-colors">
          ...
          <div className="rounded-2xl glass p-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <span className="mt-2 inline-block rounded-full border border-indigo-500/20 bg-indigo-500/15 px-3 py-1 text-sm font-medium text-indigo-300">
                  {service.category}
                </span>
              </div>
            ...
            {service.images?.length > 0 && (
              <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {service.images.map((url, i) => (
                  <img key={i} src={url} alt={`${service.title} ${i + 1}`} className="h-32 w-full rounded-xl border border-white/10 object-cover" />
                ))}
              </div>
            )}
                    ...
                    {service.provider.skills?.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {service.provider.skills.map((skill) => (
                          <span key={skill} className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-xs text-gray-300">
                            {skill}
                          </span>
                        ))}
                      </div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {user && user.role === 'customer' && (
                <Link
                  ...
                  className="rounded-xl bg-gradient-to-r from-indigo-600 to-fuchsia-600 px-6 py-3 font-semibold text-white transition-all hover:from-indigo-500 hover:to-fuchsia-500 glow-indigo"
                >
              ...
              {isOwner && (
                <>
                  <Link
                    to={`/services/edit/${id}`}
                    className="rounded-xl border border-white/10 bg-white/5 px-6 py-3 font-semibold text-white transition-colors hover:bg-white/10"
                  >
                    Edit
                  </Link>
                  <button
